...
void main(void){
	...
	// gl_Position = (vpMatrix * inverse(rtMatrix) * vec4(position,1.0)) + vec4(size * vertex_data, 0.0, 1.0) + vec4(rel_pos, 1.0);
	gl_Position = (vpMatrix * inverse(rtMatrix) * mvMatrix * (vec4(rel_pos, 1.0))) + vec4(size * vertex_data, 0.0, 0.0);
